...
SELECT `hostid` from `hosts` where `host` like  '10.1.130.82'
---创建httptest新web监控
insert into httptest(httptestid,applicationid,name,delay,status,agent,hostid)(select a.httptestid+1,'null','立体教学','60','0','Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.1; Trident/6.0)',b.hostid from httptest a,hosts b where a.httptestid in (select a1.httptestid from ( select httptestid from httptest order by httptestid desc LIMIT 0,1)as a1) and b.hostid in (select b1.hostid from ( select hostid from hosts where host like  '10.1.130.82')as b1))
---插入监控httpstep步骤
insert into httpstep(httpstepid,httptestid,name,no,url,required,status_codes)(select a.httpstepid+1,b.httptestid,'立体教学','1','http://:8086/','用户登录','200' from httpstep a,httptest b where a.httpstepid in (select a1.httpstepid from ( select httpstepid from httpstep order by httpstepid desc LIMIT 0,1)as a1) and b.httptestid in (select b1.httptestid from ( select httptestid from httptest order by httptestid desc LIMIT 0,1)as b1))
---插入6个web监控项及httptestitem和httpstepitem监控项
--httptestitem.web.test.in
insert into items (itemid,type,hostid,name,key_,delay,units) (select  itemid + '1','9','10120','Download speed for scenario "$1".','web.test.in[立体教学,,bps]','60','Bps' from items  order by itemid desc LIMIT 0,1)
...
insert into items (itemid,type,hostid,name,key_,delay,value_type) (select  itemid + '1','9','10120','Failed step of scenario "$1".','web.test.fail[立体教学]','60','3' from items  order by itemid desc LIMIT 0,1)
...
insert into items (itemid,type,hostid,name,key_,delay,value_type) (select  itemid + '1','9','10120','Last error message of scenario "$1".','web.test.error[立体教学]','60','1' from items  order by itemid desc LIMIT 0,1)
insert into httptestitem(httptestitemid,httptestid,itemid,type) (select a.httptestitemid+1,b.httptestid,c.itemid,'4' from httptestitem a,httptest b,items c where a.httptestitemid in(select a1.httptestitemid from ( select httptestitemid from httptestitem order by httptestitemid desc LIMIT 0,1)as a1) and b.httptestid in (select b1.httptestid from ( select httptestid from httptest order by httptestid desc LIMIT 0,1)as b1) and c.itemid in (select c1.itemid from ( select itemid from items order by itemid desc LIMIT 0,1)as c1))
---httpstepitem.web.test.in
insert into items (itemid,type,hostid,name,key_,delay,units) (select  itemid + '1','9','10120','Download speed for step "$2" of scenario "$1".','web.test.in[立体教学,立体教学,bps]','60','Bps' from items  order by itemid desc LIMIT 0,1)
...
insert into items (itemid,type,hostid,name,key_,delay,units) (select  itemid + '1','9','10120','Response time for step "$2" of scenario "$1".','web.test.time[立体教学,立体教学,resp]','60','s' from items  order by itemid desc LIMIT 0,1)
...
insert into items (itemid,type,hostid,name,key_,delay,value_type) (select  itemid + '1','9','10120','Response code for step "$2" of scenario "$1".','web.test.rspcode[立体教学,立体教学]','60','3' from items  order by itemid desc LIMIT 0,1)
...
---查询时间段内异常数量
SELECT count(*) from history where itemid like '1025417' and (FROM_UNIXTIME(clock,'%Y-%m-%d %H:%i:%S')>='2016-08-18 10:00:00') and (FROM_UNIXTIME(clock,'%Y-%m-%d %H:%i:%S')<='2016-08-16 00:00:00') and value like '0.0000';
---趋势图查看
SELECT itemid,FROM_UNIXTIME(clock,'%Y-%m-%d %H:%i:%S') as clock,num from trends where itemid like '1025460'
---创建异常趋势图表格
