...
----查询 主机ID
SELECT `hostid` from `hosts` where `name` like  '10.1.130.82'
...
---创建未下架产品信息，责任人邮箱视图
create view `v_zabbix_userinfo` As SELECT DISTINCT a.entity_id,b.`value` as 名称,c.value as 运维负责人,d.email,(case WHEN a.`value`='2' then 'disable' else 'enable' end) as 状态   from catalog_product_index_eav a,catalog_product_entity_varchar b ,eav_attribute_option_value as c, v_customer_info d where a.attribute_id like '135' and a.entity_id=b.entity_id  and b.attribute_id like '71' and a.value=c.option_id and c.value in ('吴鹏','陈俊聪','刘永华','江婉怡','龚小芝','柳芦','李军') and c.value=d.username ORDER BY 运维负责人 desc;

----创建zabbbix监控端口下架产品id视图
CREATE  VIEW `v_zabbix_productdown`AS select entity_id,(case WHEN `value`='2' then 'disable' else 'enable' end) as 状态 from catalog_product_entity_int where attribute_id like '96' and `value` like '2';

----创建下架产品信息视图
create view `v_zabbix_downinfo` As SELECT DISTINCT a.entity_id,b.`value` as 名称,c.value as 运维负责人,d.email,(case WHEN a.`value`!='2' then 'disable' else 'enable' end) as 状态   from catalog_product_entity_int a,catalog_product_entity_varchar b ,eav_attribute_option_value as c, v_customer_info d where a.entity_id in (select entity_id from v_zabbix_productdown) and a.entity_id=b.entity_id  and b.attribute_id like '71' and a.attribute_id like '135' and a.value=c.option_id  and  c.value=d.username ORDER BY 运维负责人 desc;
