...
create view `v_zabbix_userinfo` As SELECT DISTINCT a.entity_id,b.`value` as name,c.value as own,d.email,(case WHEN a.`value`='2' then 'disable' else 'enable' end) as status from catalog_product_index_eav a,catalog_product_entity_varchar b ,eav_attribute_option_value as c, v_customer_info d where a.attribute_id like '135' and a.entity_id=b.entity_id  and b.attribute_id like '71' and a.value=c.option_id and c.value in ('Åô','¿¡´Ï','ÓÀ»ª','âù','Ð¡Ö¥','Â«','¾ü') and c.value=d.username ORDER BY own desc;
