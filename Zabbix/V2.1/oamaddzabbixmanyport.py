   ...
   if l2<l1:
     while i1<l2:
      if b[i1] in a:
         ...
         hostname=c[0]
		 #获取zabbbix监控端口产品名称
         sql13="select name from v_zabbix_zabbixinfo where ip like '"+str(hostname)+"' and (port like '%,"+str(port)+"' or port like '%,"+str(port)+",%' or port like '"+str(port)+",%' or port like '"+str(port)+"' order by name desc LIMIT 0,1)"
   ...
   if l5<l1:
     while i3<l5:
      if bd[i3] in a:
             ...
             db2.commit()
            #获取zabbbix监控端口产品名称
             sql112="select name from v_zabbix_zabbixinfo where ip like '"+str(hostname)+"' and (port like '%,"+str(port)+"' or port like '%,"+str(port)+",%' or port like '"+str(port)+",%' or port like '"+str(port)+"')  order by name desc LIMIT 0,1"
             cursor1.execute(sql112)
             name_value=cursor1.fetchall()
             name=value(name_value)
            #获取产品运维责任人邮箱
             sql113="SELECT email from v_zabbix_downinfo where entity_id in (SELECT entity_id from v_zabbix_zabbixinfo where ip like '"+str(hostname)+"' and (port like '%,"+str(port)+"' or port like '%,"+str(port)+",%' or port like '"+str(port)+",%' or port like '"+str(port)+"'))"
