...
def emaildown (useremail,name,ip,port,status,own):
   ...
   #文件内容
   msg = MIMEText("<html><p style='color:#0099FF'><b>尊敬的运维工程师"+str(f)+"，您好：</b></p><p>&nbsp;&nbsp;产品名称："+str(b)+"</p><p>&nbsp;&nbsp;产品所在服务器IP："+str(c)+"</p><p><a详细信息：该IP服务器未添加到zabbix监控里，请添加，错误请修改。</a><a href=""http://oam.fe.com:58080/index.php/admin/index""  style='color:#0099FF'><b>运维后台入口</b></a> </p><p>&nbsp;&nbsp;监控端口："+str(d)+"<br/>&nbsp;&nbsp;<br/>&nbsp;&nbsp;</p><p>&nbsp;&nbsp;ZABBIX监控</p></html>",'html','utf-8')
   ...
   if l2<l1:
     while i1<l2:
      if b[i1] in a:
         ...
         status="(^_^)OAM-产品端口-监控正常"
         email(mail,name,hostname,port,status,own)
